...
describe('ShoppingCart e2e test', () => {
  ...
  // const shoppingCartSample = {"placedDate":"2024-06-03T14:48:19.475Z","status":"PENDING","totalPrice":11643.23,"paymentMethod":"CREDIT_CARD"};
  ...
  describe('new ShoppingCart page', () => {
    ...
    it.skip('should create an instance of ShoppingCart', () => {
      cy.get(`[data-cy="placedDate"]`).type('2024-06-04T08:24');
      cy.get(`[data-cy="placedDate"]`).blur();
      cy.get(`[data-cy="placedDate"]`).should('have.value', '2024-06-04T08:24');

      cy.get(`[data-cy="status"]`).select('COMPLETED');

      cy.get(`[data-cy="totalPrice"]`).type('26078.43');
      cy.get(`[data-cy="totalPrice"]`).should('have.value', '26078.43');

      cy.get(`[data-cy="paymentMethod"]`).select('IDEAL');

      cy.get(`[data-cy="paymentReference"]`).type('aromatic neat');
      cy.get(`[data-cy="paymentReference"]`).should('have.value', 'aromatic neat');
